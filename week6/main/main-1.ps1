...
while($operation){
    ...
    elseif($choice -eq 3){
      ...
      if(checkUser $name -eq $false){
        Write-Host "User with this name already exists" | Out-String
        continue
      }
        
      $password = Read-Host -AsSecureString -Prompt "Please enter the password for the new user"
      $passwordToConvert = $password
      $bstr = [System.Runtime.InteropServices.Marshal]::SecureStringToBSTR($passwordToConvert)
      $unsecurePassword = [System.Runtime.InteropServices.Marshal]::PtrToStringAuto($bstr)
      $unsecurePassword
      $validPassword = checkPassword $unsecurePassword
      $validPassword
      if($validPassword -eq $false){
        Write-Host "Password must be at least 6 characters long and contain`n
        at least 1 character
        at least 1 number
        at least 1 special character" | Out-String
        continue
      }
