...
logline=$(cat "$authfile" | grep "lightdm:auth"| grep "authentication failure")
output=$(echo "$logline" | tail -n 5)
echo "$output"
}
#getFailedLogins
# Sending logins as email - Do not forget to change email address
# to your own email address
echo "To: patrick.hagelston@mymail.champlain.edu" > emailform.txt
echo "Subject: Security Incident" >> emailform.txt
echo "" >> emailform.txt
getLogins >> emailform.txt
cat emailform.txt | ssmtp patrick.hagelston@mymail.champlain.edu
